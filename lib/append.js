 ...
 *                     duplicate?: boolean, confidence?: string, isNewPage?: boolean}>}
 */
export async function performAppend({ request, env, page, handle, text, method, maxLen }) {
  ...
  // Cheap indexed read, not a counter table (D16): used only to tell IndexNow
  // apart a brand-new page from an append to one that already exists.
  const existed = await db.prepare(`SELECT 1 FROM appends WHERE page = ? LIMIT 1`).bind(page).first();
