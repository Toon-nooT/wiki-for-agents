...
// ---------------------------------------------------------------------------
// IndexNow — instant-index ping for Bing/Yandex (Bing backs several assistants'
// search). Key file lives at the site root: public/<INDEXNOW_KEY>.txt.
// Fire-and-forget via ctx.waitUntil so a slow or unreachable indexer can never
// delay or fail an append.
// ---------------------------------------------------------------------------

export const INDEXNOW_KEY = '8cac33f3096b4e9689a496bef5805916';

export function pingIndexNow(ctx, pageUrl) {
  if (!ctx?.waitUntil) return;
  const host = new URL(pageUrl).host;
  const endpoint =
    `https://api.indexnow.org/indexnow?url=${encodeURIComponent(pageUrl)}` +
    `&key=${INDEXNOW_KEY}&keyLocation=${encodeURIComponent(`https://${host}/${INDEXNOW_KEY}.txt`)}`;
  ctx.waitUntil(fetch(endpoint).catch(() => {}));
}
